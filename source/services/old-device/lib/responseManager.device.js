...
class ResponseManager {
    ...
    static _processRequest(event, ticket) {
        ...
        Logger.log(Logger.levels.INFO, JSON.stringify(event));
        Logger.log(Logger.levels.INFO, 'isAdmin: ' + ticket.isAdmin);
        Logger.log(Logger.levels.INFO, 'groupid: ' + ticket.groupid);

        let _promise = null;

        if (event.resource === '/device-types' && !_.isEmpty(event.queryStringParameters) && event.httpMethod === 'GET') {
            if (event.queryStringParameters.op === 'stats') {
                _operation = 'retrieve device types stats';
                _promise = _deviceTypeManager.getDeviceTypeStats(ticket, event.queryStringParameters.page);
            } else if (event.queryStringParameters.op === 'all') {
                _operation = 'retrieve all device types ';
                _promise = _deviceTypeManager.getAllDeviceTypes(ticket);
            } else {
                _operation = 'retrieve device types';
                _promise = _deviceTypeManager.getDeviceTypes(ticket, event.queryStringParameters.page);
            }
        } else if (event.resource === '/device-types' && event.httpMethod === 'POST') {
            _operation = 'create device type';
            _promise = _deviceTypeManager.createDeviceType(ticket, _body);
        } else if (event.resource === '/device-types/{type_id}' && !(_.isEmpty(event.pathParameters)) && event.pathParameters.hasOwnProperty('type_id') && event.httpMethod === 'GET') {
            _operation = ['retrieve device type', event.pathParameters.type_id].join(' ');
            _promise = _deviceTypeManager.getDeviceType(ticket, event.pathParameters.type_id);
        } else if (event.resource === '/device-types/{type_id}' && !(_.isEmpty(event.pathParameters)) && event.pathParameters.hasOwnProperty('type_id') && event.httpMethod === 'PUT') {
            _operation = ['update device type', event.pathParameters.type_id].join(' ');
            _promise = _deviceTypeManager.updateDeviceType(ticket, event.pathParameters.type_id, _body);
        } else if (event.resource === '/device-types/{type_id}' && !(_.isEmpty(event.pathParameters)) && event.pathParameters.hasOwnProperty('type_id') && event.httpMethod === 'DELETE') {
            _operation = ['delete device type', event.pathParameters.type_id].join(' ');
            _promise = _deviceTypeManager.deleteDeviceType(ticket, event.pathParameters.type_id);
        } else if (event.resource === '/devices' && event.httpMethod === 'GET') {
            if (event.queryStringParameters.op === 'stats') {
                _operation = 'retrieve device stats for group ' + ticket.groupid;
                _promise = _deviceManager.getDeviceStats(ticket, event.queryStringParameters.filter);
            } else {
                _operation = 'retrieve devices for group ' + ticket.groupid;
                _promise = _deviceManager.getDevices(ticket, event.queryStringParameters.filter, event.queryStringParameters.page);
            }
        } else if (event.resource === '/devices' && event.httpMethod === 'POST') {
            _operation = 'create device for group' + ticket.groupid;
            _promise = _deviceManager.createDevice(ticket, _body);
        } else if (event.resource === '/devices/{device_id}' && !(_.isEmpty(event.pathParameters)) && event.pathParameters.hasOwnProperty('device_id') && event.httpMethod === 'GET') {
            _operation = ['retrieve device', event.pathParameters.device_id].join(' ');
            _promise = _deviceManager.getDevice(ticket, event.pathParameters.device_id);
        } else if (event.resource === '/devices/{device_id}' && !(_.isEmpty(event.pathParameters)) && event.pathParameters.hasOwnProperty('device_id') && event.httpMethod === 'PUT') {
            _operation = ['update device', event.pathParameters.device_id].join(' ');
            _promise = _deviceManager.updateDevice(ticket, event.pathParameters.device_id, _body);
        } else if (event.resource === '/devices/{device_id}' && !(_.isEmpty(event.pathParameters)) && event.pathParameters.hasOwnProperty('device_id') && event.httpMethod === 'DELETE') {
            _operation = ['delete device', event.pathParameters.device_id].join(' ');
            _promise = _deviceManager.deleteDevice(ticket, event.pathParameters.device_id);
        }
        ...
        if (_promise === null) {
            ...
            return _promise
                .then(data => this._processResponse(200, data, _operation))
                .catch(err => this._processResponse(err.code, err, _operation));
        }
        ...
        // return new Promise((resolve, reject) => {
        //     if (event.resource.startsWith('/device-types')) {
        //         if (_.isEmpty(event.pathParameters) && event.httpMethod === 'GET') {
        //             _operation = 'retrieve device types';
        //             Logger.log(Logger.levels.INFO, ['Attempting to', _operation].join(' '));
        //             _deviceTypeManager.getDeviceTypes(ticket).then((data) => {
        //                 resolve(this._processResponse(200, data, _operation));
        //             }).catch((err) => {
        //                 reject(this._processResponse(err.code, err, _operation));
        //             });
        //         } else if (_.isEmpty(event.pathParameters) && event.httpMethod === 'POST' && isAdmin) {
        //             _operation = 'create device type';
        //             Logger.log(Logger.levels.INFO, ['Attempting to', _operation].join(' '));
        ...
        //                 reject(this._processResponse(err.code, err, _operation));
        //             });
        //         } else if (!(_.isEmpty(event.pathParameters)) && event.pathParameters.hasOwnProperty('typeid')) {
        ...
        //                     reject(this._processResponse(err.code, err, _operation));
        //                 });
        //             } else if (event.httpMethod === 'PUT' && isAdmin) {
        ...
        //                     reject(this._processResponse(err.code, err, _operation));
        //                 });
        //             } else if (event.httpMethod === 'DELETE' && isAdmin) {
        ...
        //             _operation = 'retrieve device widgets for group';
        //             Logger.log(Logger.levels.INFO, ['Attempting to', _operation].join(' '));
