...
class User {
    ...
    /**
     * Retrieves a group from the Amazon Cognito user pool.
     * @param {integer} groupId - GroupName of group to retrieve from the user pool.
     */
    getGroup(groupId) {
    ...
    /**
     * Retrieves users for a given group from Amazon Cognito user pool.
    ...
    getUsersForGroup(groupId) {
        ...
        return new Promise((resolve, reject) => {
            this._getUserPoolConfigInfo().then((poolinfo) => {
                ...
                Logger.log(Logger.levels.INFO, 'getUsersForGroup.listUsersInGroup(' + JSON.stringify(params) + ')');
                ...
                cognitoidentityserviceprovider.listUsersInGroup(params, function (err, data) {
                    if (err) {
                        ...
                        reject({
                            code: 500,
                            error: 'listUsersInGroup',
                            message: `Error occurred while attempting to list users for group in the user pool.`
                        });
                    } else {
                        Logger.log(Logger.levels.INFO, 'getUsersForGroup.listUsersInGroup: data = ' + JSON.stringify(data));
                        let _users = [];
                        if (!_.isEmpty(data.Users)) {
                            for (let i = 0; i < data.Users.length; i++) {
                                let _user = {
