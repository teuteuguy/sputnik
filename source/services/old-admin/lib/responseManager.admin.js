...
class ResponseManager {
    ...
    static _processRequest(event, ticket) {
        ...
        return new Promise((resolve, reject) => {
            ...
            // } else if (event.resource === '/admin/groups' && event.httpMethod === 'GET' && isAdmin) {
            } else if (event.resource.startsWith('/admin/groups') && isAdmin) {
                ...
                } else if (!(_.isEmpty(event.pathParameters)) && event.pathParameters.hasOwnProperty('group_id') && event.resource.endsWith('/users') && event.httpMethod === 'GET') {
                    _operation = ['retrieve users for group', event.pathParameters.group_id].join(' ');
                    Logger.log(Logger.levels.INFO, ['Attempting to', _operation].join(' '));
                    _user.getUsersForGroup(event.pathParameters.group_id).then((data) => {
                        resolve(this._processResponse(200, data, _operation));
                    }).catch((err) => {
                        reject(this._processResponse(500, err, _operation));
                    });
                } else if (!(_.isEmpty(event.pathParameters)) && event.pathParameters.hasOwnProperty('group_id') && event.httpMethod === 'GET') {
                    _operation = ['retrieve group', event.pathParameters.group_id].join(' ');
                    Logger.log(Logger.levels.INFO, ['Attempting to', _operation].join(' '));
                    _user.getGroup(event.pathParameters.group_id).then((data) => {
                        resolve(this._processResponse(200, data, _operation));
                    }).catch((err) => {
                        reject(this._processResponse(500, err, _operation));
                    });
